...
def get_pod_resource_list(obj: Dict[str, Any], **kwargs: Any) -> Tuple[List[Dict[str, Any]], Union[int, str, List[StatusGroup]]]:
	...
	# OK, we have the controller kind (and thus trivy_selector); time to fire off the requests for lists
	for resource, kwargs in (("event",
				  {"kind": ("Event", ""), "namespace": pod_namespace,
				   "field_selector": make_selector({"involvedObject.name": pod_name, "involvedObject.namespace": pod_namespace})}),
				 ("pod_disruption_budget",
				  {"kind": ("PodDisruptionBudget", "policy"), "namespace": pod_namespace}),
				 ("pod_metrics",
				  {"kind": ("PodMetrics", "metrics.k8s.io")}),
				 ("config_audit_report",
				  {"kind": ("ConfigAuditReport", "aquasecurity.github.io"), "namespace": pod_namespace, "label_selector": trivy_selector}),
				 ("infra_assessment_report",
				  {"kind": ("InfraAssessmentReport", "aquasecurity.github.io"), "namespace": pod_namespace, "label_selector": trivy_selector}),
				 ("vulnerability_report",
				  {"kind": ("VulnerabilityReport", "aquasecurity.github.io"), "namespace": pod_namespace, "label_selector": trivy_selector}),
				 ("exposed_secret_report",
				  {"kind": ("ExposedSecretReport", "aquasecurity.github.io"), "namespace": pod_namespace, "label_selector": trivy_selector}),
				 ("sbom_report",
				  {"kind": ("SbomReport", "aquasecurity.github.io"), "namespace": pod_namespace, "label_selector": trivy_selector}),
				 ("antrea_agent",
				 ...
				 ("resourceclaim",
				  {"kind": ("ResourceClaim", "resource.k8s.io"), "namespace": pod_namespace}),
				):
		kind = deep_get(kwargs, DictPath("kind"))
	...
	if "service_account" not in filter_resources and deep_get(obj, DictPath("spec#serviceAccountName")) is not None:
		kind = ("ServiceAccount", "")
		rtype = resource_kind_to_rtype(kind)
	...
	for vol in deep_get(obj, DictPath("spec#volumes"), []):
		...
		if deep_get(vol, DictPath("configMap")) is not None and "config_map" not in filter_resources:
			kind = ("ConfigMap", "")
			rtype = resource_kind_to_rtype(kind)
...
def listgetter_ansible_volumes(obj: Dict, **kwargs: Any) -> Tuple[List[Dict], str]:
	...
	for item in deep_get(obj, DictPath("ansible_mounts"), []):
		...
		if check_matchlist(mountpoint, deep_get(cmtlib.cmtconfig, DictPath("__Inventory#mountpoint_skiplist"),
...
# pylint: disable-next=unused-argument
def listgetter_matchrules(obj: Dict, **kwargs: Any) -> Tuple[List[Dict], Union[str, int, List[StatusGroup]]]:
	"""
