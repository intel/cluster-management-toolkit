...
class CMTLog:
    """
    A structured log format used by CMT that serialises to YAML on disk.
    To ensure that we can use append we use a list;
    this means that the performance is worse when reading the log,
    ...
    and the number of log files limited to 100,
    hence setting an upper limit to 100MB worth of logs.
    """
    log: List[Dict] = []
    logtype = None
    ...
    def __rate_limit(self, filepath: FilePath,
                     rate_limit: int, exception_on_flood: bool = True) -> bool:
        now = datetime.now()
        ...
        # More than 10 log messages in a minute is an indication
        # that something is very, very wrong; this should be close to 0
        if self.writes_last_minute > rate_limit:
            if exception_on_flood:
                raise ProgrammingError(f"The log {filepath} is flooded; "
                                       "more than {rate_limit} messages in 60 seconds")
            return False
    ...
    def __format_entry(self, message: Union[List[List[ANSIThemeStr]], List[str]],
                       ...
                       function: str = "", lineno: int = 0) -> Dict:
        ...
        if not isinstance(message, list):
            raise TypeError("CMTLog only accepts log messages as "
                            f"List[str] or List[themearray]; received {type(message)}")
    ...
    def __rotate_filename(self, dirpath: FilePath, basename: str, suffix: str = "",
                          maxsize: int = 1024 ** 3) -> Optional[FilePath]:
    ...
    def __init__(self, path: Union[CMTLogType, FilePath],
        ...
        log_entry = self.__format_entry(message, severity, timestamp,
                                        facility, file, function, lineno)

        # The audit log is always synchronous
        if path == CMTLogType.AUDIT:
            log_path = self.__rotate_filename(dirpath=CMT_LOGS_DIR,
                                              basename=AUDIT_LOG_BASENAME,
    ...
    def add(self, message: Union[List[List[ANSIThemeStr]], List[str]],
        ...
        # The audit log is always synchronous; calling it like this is incorrect,
        # but we want audit messages to be successful, so we just log an extra message
        if self.logtype == CMTLogType.AUDIT:
            warning_log_entry = {
                "timestamp": timestamp,
                ...
                "lineno": lineno,
                "ppid": os.getppid(),
                "strarray": ["CMTLog.add() called from a synchronous log; "
    ...
    def flush(self) -> None:
        ...
        if self.logtype == CMTLogType.AUDIT:
            ...
            warning_log_entry = {
                ...
                "lineno": lineno,
                "ppid": os.getppid(),
                "strarray": ["CMTLog.flush() called from a synchronous log; "
                             "this is a programming error."]
            }
            log_path = self.__rotate_filename(dirpath=CMT_LOGS_DIR,
                                              basename=AUDIT_LOG_BASENAME,
    ...
    def close(self) -> None:
        """
        Close the log; this is not necessary from an I/O-perspective;
        it is intended to be used to detect logical errors in the code
        """
        self.logtype = None
        self.log = []
