...
def get_pod_info(**kwargs: Any) -> List[Type]:
	...
	for obj in vlist:
		...
		if in_depth_node_status:
			...
			containers = get_containers(containers = deep_get(obj, DictPath("spec#initContainers"), []),
						    container_statuses = deep_get(obj, DictPath("status#initContainerStatuses"), []))
			containers += get_containers(containers = deep_get(obj, DictPath("spec#containers"), []),
