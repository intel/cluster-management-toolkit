					...
					for version in versions:
						...
						for resource in resources:
							...
							if (kind_, name) in kubernetes_resources and \
									f"apis/{name}/{version_}/" in cast(List[str], kubernetes_resources[(kind_, name)].get("api_paths", [])):
				...
				for version in versions:
					...
					for resource in deep_get(data, DictPath("resources"), []):
						...
						if not (kind_ := deep_get(resource, DictPath("kind"), "")):
							continue
						if (kind_, name) in kubernetes_resources and f"apis/{version_}/" in cast(List[str], kubernetes_resources[(kind_, name)].get("api_paths", [])):
